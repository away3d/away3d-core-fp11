package away3d.managers
{
	 ...
	 */ public class Stage3DProxy extends EventDispatcher
	{
        ...
        public function activateTexture(index:int, textureProxy:Texture2DBase):void
        {
            var textureData:TextureData = _texturePool.getItem(textureProxy);
            ...
            if (textureData.invalid) {
                ...
                if (textureProxy.generateMipmaps) {
                    var mipmapData:Vector.<BitmapData> = textureProxy.getMipmapData();
                    var len:int = mipmapData.length;
                    for (var i:int = 0; i < len; i++)
                        (textureData.texture as Texture).uploadFromBitmapData(mipmapData[i], i);
                } else {
                    (textureData.texture as Texture).uploadFromBitmapData(textureProxy.getTextureData(), 0);
                }
            }
        ...
        public function activateCubeTexture(index:int, textureProxy:CubeTextureBase):void
        {
            var textureData:TextureData = _texturePool.getItem(textureProxy);
            ...
            if (textureData.invalid) {
                ...
                for (var i:int = 0; i < 6; ++i) {
                    if (textureProxy.generateMipmaps) {
                        var mipmapData:Vector.<BitmapData> = textureProxy.getMipmapData(i);
                        var len:int = mipmapData.length;
                        for (var j:int = 0; j < len; j++)
                            (textureData.texture as CubeTexture).uploadFromBitmapData(mipmapData[j], i, j);
                    } else {
                        (textureData.texture as CubeTexture).uploadFromBitmapData(textureProxy.getTextureData(i), i, 0);
                    }
                }
            }
