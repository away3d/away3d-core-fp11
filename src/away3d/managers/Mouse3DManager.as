package away3d.managers
{
	...
	public class Mouse3DManager
	{
		...
		{
			if (collidingObject)
				queueDispatch(_mouseMove, _mouseMoveEvent = event); else
				reThrowEvent(event);
			updateDirty = true;
		}
		...
		private function onMouseOver(event:MouseEvent):void
		{
			...
			if (collidingObject && _previousCollidingObject != collidingObject)
				queueDispatch(_mouseOver, event, collidingObject); else
				reThrowEvent(event);
			updateDirty = true;
		}
		...
		{
			if (collidingObject)
				queueDispatch(_mouseDoubleClick, event); else
				reThrowEvent(event);
			updateDirty = true;
		}
		...
		{
			if (collidingObject)
				queueDispatch(_mouseWheel, event); else
				reThrowEvent(event);
			updateDirty = true;
		}
