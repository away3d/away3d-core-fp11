	...
	public class LatheExtrude extends Mesh
	{
    	...
    	*/
		...
		private function generate(vectors:Vector.<Vector3D>, axis:String, tweek:Object, render:Boolean = true, id:uint = 0):void
		{
			...
			if(!_varr) _varr = new Vector.<Vertex>();
			
			for (i = 0; i < vectors.length; ++i) {
				_varr.push(new Vertex(vectors[i].x, vectors[i].y, vectors[i].z) ); 
			...
			for (i = 0; i <= _subdivision * _revolutions ; ++i) {
				...
				for(j = 0;j<tmpVecs.length;++j){
					...
					_varr.push(new Vertex(tmpVecs[j].x, tmpVecs[j].y, tmpVecs[j].z) );
		...
		private function addFace(v0:Vertex, v1:Vertex, v2:Vertex, uv0:UV, uv1:UV, uv2:UV, subGeomInd:uint):void
		{
