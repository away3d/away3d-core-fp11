	...
	public class ParserBase extends EventDispatcher
	{
		...
		*/
										  		...
										  		weights : Vector.<Number>, jointIndices : Vector.<Number>) : CompactSubGeometry
		{
			...
			constructVertexData(sub, verts, uvs, normals, tangents);
			return sub;
		}

		protected function constructVertexData(sub : CompactSubGeometry, verts : Vector.<Number>, uvs : Vector.<Number>, normals : Vector.<Number>, tangents : Vector.<Number>) : void
		{
			...
			sub.updateData(data);
			sub.autoDeriveVertexNormals = !(normals && normals.length);
			sub.autoDeriveVertexTangents = !(tangents && tangents.length);
			sub.autoGenerateDummyUVs = !(uvs && uvs.length);
