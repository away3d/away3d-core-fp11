	...
	public class AWD2Parser extends ParserBase
	{
		...
		private function parseSkeleton(blockLength : uint) : Skeleton
		{
			...
			while (joints_parsed < num_joints) {
				...
				var joint_mtx_data : Vector.<Number>;
				var joint : Joint;
				...
				joint.inverseBindPose = parseMatrixRawData();
				
				// TODO: REmove hack
				if (joint.parentIndex == 1) {
					var mtx : Matrix3D = new Matrix3D(joint.inverseBindPose);
					mtx.invert();
					mtx.prependTranslation(0, -100, 0);
					mtx.invert();
					
					joint.inverseBindPose = mtx.rawData;
				}
				
				/*
				joint_mtx_data = parseMatrixRawData();
				var mtx : Matrix3D = new Matrix3D;
				var qt : Quaternion = new Quaternion;
				qt.x = joint_mtx_data[3];
				qt.y = joint_mtx_data[4];
				qt.z = joint_mtx_data[5];
				qt.w = joint_mtx_data[6];
				qt.toMatrix3D(mtx);
				
				mtx.position.x = joint_mtx_data[0];
				mtx.position.y = joint_mtx_data[1];
				mtx.position.z = joint_mtx_data[2];
				...
				*/
				...
				// Discard props and attr
				// TODO: Implement these once added to libawd. Should be since they are in spec
				//parseProperties();
				//parseUserAttributes();
		...
		private function parseSkeletonPose(blockLength : uint) : SkeletonPose
		{
			...
			pose = new SkeletonPose(num_joints);
			...
			while (joints_parsed < num_joints) {
				...
				if (has_transform == 1) {
					...
					if (joints_parsed == 1) {
						trace(joint_pose.translation);
					}
					/*
					joint_pose.orientation.x = mtx_data[0];
					joint_pose.orientation.y = mtx_data[1];
					joint_pose.orientation.z = mtx_data[2];
					joint_pose.orientation.w = mtx_data[3];
					joint_pose.translation.x = mtx_data[4];
					joint_pose.translation.y = mtx_data[5];
					joint_pose.translation.z = mtx_data[6];
					*/
		...
		{
			return new Matrix3D(parseMatrixRawData());
		}
		
		private function parseMatrixRawData() : Vector.<Number>
		{
			var i : uint;
			var mtx_raw : Vector.<Number>;
