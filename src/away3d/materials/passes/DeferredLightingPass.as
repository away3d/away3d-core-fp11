package away3d.materials.passes {
	...
	public class DeferredLightingPass extends MaterialPassBase {
		...
		public var lightAccumulation:Texture2DBase;
		public var lightAccumulationSpecular:Texture2DBase;
		...
		override public function getFragmentCode(shaderObject:ShaderObjectBase, registerCache:ShaderRegisterCache, sharedRegisters:ShaderRegisterData):String {
			...
			if (useDeferredSpecularLighting) {
				if (specularMap) {
...
//				context3D.setTextureAt(_shader.getTexture(SPECULAR_TEXTURE), specularMap.getTextureForStage3D(stage3DProxy));
			...
			if (_shader.hasTexture(ACCUMULATION_TEXTURE) && lightAccumulation) {
				stage.activateTexture(_shader.getTexture(ACCUMULATION_TEXTURE), lightAccumulation);
//				context3D.setTextureAt(_shader.getTexture(ACCUMULATION_TEXTURE), _material.lightAccumulation.getTextureForStage3D(stage3DProxy));
			...
			if (_shader.hasTexture(ACCUMULATION_SPECULAR_TEXTURE) && lightAccumulationSpecular) {
				stage.activateTexture(_shader.getTexture(ACCUMULATION_SPECULAR_TEXTURE), lightAccumulationSpecular);
