package away3d.materials.methods
{
	 ...
	 */
	public class DiffuseSubSurfaceMethod extends DiffuseCompositeMethod
	{
		 ...
		 */
		public function DiffuseSubSurfaceMethod(depthMapSize:int = 512, depthMapOffset:Number = 15)
		{
			super(scatterLight);
		...
		override arcane function initConstants(shaderObject:ShaderObjectBase, methodVO:MethodVO):void
		{
			...
			var data:Vector.<Number> = shaderObject.vertexConstantData;
			var index:int = methodVO.secondaryVertexConstantsIndex;
			data[index] = .5;
			data[index + 1] = -.5;
			data[index + 2] = 0;
			data[index + 3] = 1;
		...
		{
			var code:String = super.getVertexCode(shaderObject, methodVO, registerCache, sharedRegisters);
			...
			_lightProjVarying = registerCache.getFreeVarying();
		...
		{
			_colorReg = registerCache.getFreeFragmentConstant();
			_decReg = registerCache.getFreeFragmentConstant();
			_propReg = registerCache.getFreeFragmentConstant();
            methodVO.secondaryFragmentConstantsIndex = _colorReg.index*4;
		...
		arcane override function getFragmentCodePerLight(shaderObject:ShaderLightingObject, methodVO:MethodVO, lightDirReg:ShaderRegisterElement, lightColReg:ShaderRegisterElement, registerCache:ShaderRegisterCache, sharedRegisters:ShaderRegisterData):String
		{
			...
			return super.getFragmentCodePerLight(shaderObject, methodVO, lightDirReg, lightColReg, registerCache, sharedRegisters);
		}
		...
		{
			var code:String = super.getFragmentPostLightingCode(shaderObject, methodVO, targetReg, registerCache, sharedRegisters);
			...
			if (_targetReg != sharedRegisters.viewDirFragment)
		 ...
		 * @inheritDoc
		 */
		arcane override function setRenderState(shaderObject:ShaderLightingObject, methodVO:MethodVO, renderable:RenderableBase, stage:Stage3DProxy, camera:Camera3D):void
		{
			stage.activateTexture(methodVO.secondaryTexturesIndex, _depthPass.getDepthMap(renderable));
            _depthPass.getProjection(renderable).copyRawDataTo(shaderObject.vertexConstantData, methodVO.secondaryVertexConstantsIndex + 4, true);
		}
