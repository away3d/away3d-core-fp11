package away3d.core.render {
	...
	public class DirectionalLightRenderer {
		...
		private var _directionalLightsLength:uint;
		...
		public function render(stage3DProxy:Stage3DProxy, entityCollector:EntityCollector, hasMRTSupport:Boolean, frustumCorners:Vector.<Number>, normalTexture:Texture2DBase, depthTexture:Texture2DBase, specularTexture:Texture2DBase):void {
			...
			if (entityCollector.directionalLights.length != _directionalLightsLength) {
				_directionalLights = entityCollector.directionalLights;
				_directionalLightsLength = _directionalLights.length;
				dirtyPrograms[index] = true;
			}
			...
			var k:int = 0;
			for (i = 0; i < _directionalLightsLength; i++) {
				var light:DirectionalLight = _directionalLights[i];
				var dir:Vector3D = _camera.inverseSceneTransform.deltaTransformVector(light.sceneDirection);
				var dx:Number = dir.x;
				var dy:Number = dir.y;
				var dz:Number = dir.z;
				var len:Number = -1 / Math.sqrt(dx * dx + dy * dy + dz * dz);
				_lightData[k++] = dir.x * len;
				_lightData[k++] = dir.y * len;
				_lightData[k++] = dir.z * len;
				_lightData[k++] = 1;
		...
		private function compileFragmentProgram():void {
			...
			var lightDataStart:int = _shader.getFragmentConstant(LIGHT_FC, _directionalLightsLength * 3);
			for (var i:uint = 0; i < _directionalLightsLength; i++) {
				var lightData:int = lightDataStart + i * 3;
				var temp:int = _shader.getFreeFragmentTemp();
				var nDotL:int = _shader.getFreeFragmentTemp();
				//DIFFUSE
				_fragmentCode += "dp3 ft" + temp + ".x, ft" + normal + ".xyz, fc" + lightData + ".xyz\n";
