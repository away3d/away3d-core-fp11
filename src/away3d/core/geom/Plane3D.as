package away3d.core.geom
{
	...
	public class Plane3D
	{
		...
		public function Plane3D(a:Number = 0, b:Number = 0, c:Number = 0, d:Number = 0)
		{
			...
			this.d = d;
			if (a == 0 && b == 0)
				_alignment = ALIGN_XY_AXIS; else
				if (b == 0 && c == 0)
					_alignment = ALIGN_YZ_AXIS; else
					if (a == 0 && c == 0)
						_alignment = ALIGN_XZ_AXIS; else
		...
		public function distance(p:Vector3D):Number
		{
			if (_alignment == ALIGN_YZ_AXIS)
				return a * p.x - d; else
				if (_alignment == ALIGN_XZ_AXIS)
					return b * p.y - d; else
					if (_alignment == ALIGN_XY_AXIS)
						return c * p.z - d; else
		...
		public function classifyPoint(p:Vector3D, epsilon:Number = 0.01):int
		{
			...
			if (len < -epsilon)
				return PlaneClassification.BACK; else
				if (len > epsilon)
