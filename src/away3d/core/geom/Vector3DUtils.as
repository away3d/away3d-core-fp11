package away3d.core.geom
{
	...
	/**
	 * Vector3DUtils provides additional Vector3D geom functions.
	 */
	public class Vector3DUtils
	{
		...
		public static function matrix2euler(m1:Matrix3D):Vector3D
		{
			...
			if (Math.round(result.z / MathPI) == 1) {
				if (result.y > 0)
					result.y = -(result.y - MathPI); else
					result.y = -(result.y + MathPI);

				result.z -= MathPI;

				if (result.x > 0)
					result.x -= MathPI; else
					result.x += MathPI;
			} else
				if (Math.round(result.z / MathPI) == -1) {
					if (result.y > 0)
					...
					if (Math.round(result.x / MathPI) == 1) {
						if (result.y > 0)
							result.y = -(result.y - MathPI); else
							result.y = -(result.y + MathPI);
